...
with open("data/00_formats.sas", "w") as format_file:
    format_file.write("proc format;\n")

wb = load_workbook("data/datamap.xlsx", data_only=True)
ws = wb["Sheet1"]
data = {}
i = 0
for row in ws.iter_rows(min_row=2, min_col=2):
    ...
    if type_variable.lower() == "single answer" and min_num is None and max_num is None:
        ...
        for key, value in data.items():
            ...
            print(result_items, value_items)
...
for key, value in data.items():
    with open("data/00_formats.sas", "a", encoding="utf-8") as format_file1:
        format_file1.write(
            f"value {key}\n"
        )
    for key1, value1 in value.items():
        ...
        with open("data/00_formats.sas", "a", encoding="utf-8") as format_file2:
            format_file2.write(
                f"{key1.split(' ')[1]}='{value1}'\n"
            )
    with open("data/00_formats.sas", "a", encoding="utf-8") as format_file3:
        format_file3.write(";\n")


with open("data/00_formats.sas", "a", encoding="utf-8") as format_file4:
    format_file4.write(
        "data a;\n set a;\n format \n "
    )
...
for key2, value2 in data.items():
    for var_value in value2["variables"]:
        with open("data/00_formats.sas", "a", encoding="utf-8") as format_file5:
            format_file5.write(f"{var_value}\n")
    with open("data/00_formats.sas", "a", encoding="utf-8") as format_file6:
